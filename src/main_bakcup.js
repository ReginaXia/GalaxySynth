...
// Background controller: 背景跟 lead 走，并保证“纯主色/不盖星云”
const bgController = createBackgroundController(bg, { steps: STEPS });
// 可选：指定你要的纯主色（也可以改成 GUI 控制）
bgController.setMainColor("#ff7ccf");
...
function tick() {
  ...
  // -------------------- Dreamy background (via controller) --------------------
  bgController.update({
    t,
    dt,
    camera,
